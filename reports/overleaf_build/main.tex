...
\toprule
\rowcolor{EHTPTableHead}Objectif & Indicateur & Cible & Résultat final \\
\midrule
Classification multi-classe équilibrée & F1-Macro & $\geq$ 0,72 & 0,679 (Voting RF+XGB) \\
Détection précoce maintenance & Recall \textit{needs repair} & $\geq$ 0,65 & 0,685 (XGB SMOTE+seuil), objectif atteint \\
Pipeline temps réel & Latence inférence & < 5 s & 22-70 ms \\
Validation automatisée & Tests pytest & $\geq$ 80 \% & 100 \% (37/37) \\
...
\subsection{Problem Statement (formulation du problème)}
...
\subsection{Le problème posé par nos données}
...
\toprule
\rowcolor{EHTPTableHead}Classe & Proportion \\
\midrule
functional & 54,3 \% \\
non functional & 38,4 \% \\
...
\subsection{Définitions (par classe)}
...
\toprule
\rowcolor{EHTPTableHead}Métrique & Formule & Lecture métier \\
\midrule
...
\subsection{F1-Macro : ce n'est pas « le » F1-score}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Terme & Signification \\
\midrule
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Classe & Pompes (support) & F1 de la classe \\
\midrule
...
\subsection{Pourquoi nous n'avons pas retenu l'accuracy comme métrique principale}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Modèle naïf & Accuracy & Recall needs repair \\
\midrule
...
\subsection{Pourquoi nous n'avons pas retenu la précision seule}
...
\subsection{Pourquoi nous n'avons pas retenu le recall seul}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Métrique & Valeur \\
\midrule
...
\subsection{Synthèse : métriques retenues et rôle de chacune}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Métrique & Rôle dans le projet & Retenue comme objectif principal ? \\
\midrule
\textbf{F1-Macro} & Comparer ML, DL et voting de façon équitable sur 3 classes & \textbf{Oui} (objectif scientifique $\geq$ 0,72) \\
\textbf{Recall \textit{needs repair}} & Mesurer la détection des pompes dégradées (priorité terrain Maroc) & \textbf{Oui} (objectif métier $\geq$ 0,65) \\
...
\subsection{Deux objectifs, deux modèles champions (S5)}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Stratégie & Effet \\
\midrule
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Usage & Modèle & Métrique clé & Valeur \\
\midrule
...
\subsection{Objectifs chiffrés et bilan}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Métrique & Seuil fixé & Résultat & Statut \\
\midrule
F1-Macro & $\geq$ 0,72 & 0,679 (Voting RF+XGB) & Non atteint \\
Recall needs repair & $\geq$ 0,65 & 0,685 (XGB SMOTE + seuil) & Atteint \\
\bottomrule
\end{longtable}
}
...
\subsection{Explication de l'origine des seuils numériques (pourquoi 0,72, 0,65, etc.)}

Les valeurs cibles ne sont pas tirées d'une norme ONEE ni d'un texte réglementaire marocain. Elles ont été \textbf{fixées au cadrage du projet} (Sprint 1, Problem Statement dans \texttt{notebooks/01\_eda.ipynb}) et reprises dans le \textbf{backlog du projet} (\texttt{AquaSense\_AI\_Sprints\_Backlog.md}, critères d'acceptation S3, S5 et S8). Elles servent de \textbf{références internes} pour juger si le pipeline est exploitable. A notre connaisance il n'en n'existe pas selon la strategie marocain ni selon des loi international de l'IA.
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Seuil & Valeur & D'où vient-il ? & Pourquoi cette valeur ? \\
\midrule
\textbf{F1-Macro} & $\geq$ 0,72 & Backlog S3/S5 + EDA §9 & Exiger une performance \textbf{équilibrée} sur les 3 classes, pas seulement sur la classe majoritaire. Le plaford observé en ML classique sur Pump It Up est d'environ \textbf{0,67} (RF tuned, S3). Le seuil \textbf{0,72} est donc \textbf{légèrement au-dessus} de ce que nous avons effectivement atteint en baseline, pour viser un modèle « bon » sans viser le niveau des gagnants DrivenData (> 0,80 avec feature engineering poussé). C'est un objectif \textbf{ambitieux mais réaliste} pour un projet de semestre. \\
\textbf{Recall \textit{needs repair}} & $\geq$ 0,65 & Backlog S3/S5/S8 + EDA §9 & La classe critique ne représente que \textbf{7 \%} des pompes. Détecter \textbf{65 \%} des vraies pompes dégradées signifie en repérer environ \textbf{deux tiers} avant panne complète. Au Maroc, \textbf{rater une dégradation} (faux négatif) est plus coûteux qu'une fausse alerte (déplacement technicien inutile) : d'où un seuil de rappel \textbf{élevé} plutôt qu'un seuil de précision. La valeur \textbf{0,65} est aussi proche du recall natif du XGBoost tuned (\textbf{0,64}, S3) avant recall boost. \\
\textbf{Latence MQTT $\rightarrow$ prédiction} & < 5 s & Backlog S6 & Contrainte \textbf{temps réel} pour la démo IoT : le technicien doit voir une prédiction peu après la télémétrie. Nos mesures (\textbf{22-70 ms}) sont très en dessous de ce plafond. \\
...
\textbf{Tests pytest} & $\geq$ 80 \% de réussite & Backlog S8 & Standard minimal de fiabilité logicielle pour un livrable reproductible. Nous avons atteint \textbf{100 \%} (37/37). \\
...
\subsection{Comparaison données Maroc vs Pump It Up}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Critère & Données Maroc (ONEE, ABH) & Pump It Up (Tanzanie) \\
\midrule
...
Reproductibilité & Difficile & Oui \texttt{download\_data.py} \\
...
\subsection{Analogie conceptuelle Maroc ↔ Tanzanie}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Concept & Maroc & Pump It Up \\
\midrule
...
\subsection{Ce qui est conservé vs adapté}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Élément & Statut \\
\midrule
...
\subsection{Pipeline de bout en bout}
...
[Dataset proxy : Pump It Up : 59 400 pompes labellisées]
  ...
  S5 Arbitrage ──► champion\_production\_v1.joblib
                 │
                 ▼
  S6 MQTT ──► Simulateur $\rightarrow$ Mosquitto $\rightarrow$ Consumer $\rightarrow$ SQLite
                 │
                 ▼
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Couche & Technologies \\
\midrule
...
├── notebooks/             \# 00\_setup $\rightarrow$ 05\_comparison\_final
...
\chapter[Sprint 0]{Sprint 0 : Setup et environnement}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Livrable & Fichier \\
\midrule
...
\subsection{Dataset Pump It Up}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Fichier & Taille & Rôle \\
\midrule
...
\subsection{Audit initial validé}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Dataset & Shape attendue & Shape obtenue \\
\midrule
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Classe & Count & \% \\
\midrule
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Colonne & NaN & \% \\
\midrule
...
\chapter[Sprint 1]{Sprint 1 : Acquisition et analyse exploratoire}
...
\subsection{Données clés confirmées}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Élément & Valeur \\
\midrule
...
\subsection{Visualisations produites (12+ graphiques)}
...
\subsection{Stratégie d'imbalance (décision S1 $\rightarrow$ S3)}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Approche & Usage \\
\midrule
...
Métriques & F1-Macro $\geq$ 0,72 ,  Recall needs repair $\geq$ 0,65 \\
\bottomrule
\end{longtable}
}
...
\subsection{Catalogue anomalies $\rightarrow$ actions S2}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Anomalie EDA & Traitement S2 \\
\midrule
...
\chapter[Sprint 2]{Sprint 2 : Wrangling et feature engineering}

\label{ch:sprint2}

\subsection{Objectif et livrables}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Livrable & Détail \\
\midrule
...
\subsection{Pattern fit/transform : anti data leakage}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Colonne & Raison \\
\midrule
...
\subsection{Feature engineering (8 features dérivées)}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Feature & Formule / logique & Motivation \\
\midrule
...
\subsection{Encodage catégoriel (au moment de l'entraînement S3)}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Type & Colonnes & Encodage & Modèles \\
\midrule
...
\subsection{Résultats finaux S2}
...
\subsection{Ordre des opérations pipeline}

\begin{lstlisting}

\_fix\_gps $\rightarrow$ \_fix\_construction\_year $\rightarrow$ \_fix\_gps\_height $\rightarrow$ \_fix\_funder\_installer
$\rightarrow$ \_fix\_amount\_tsh $\rightarrow$ \_engineer\_features $\rightarrow$ \_encode\_binary $\rightarrow$ \_drop\_columns
...
\chapter[Sprint 3]{Sprint 3 : Machine Learning classique}

\label{ch:sprint3}

\subsection{Objectif}

Entraîner et comparer 4 baselines + GridSearch RF/XGB, établir la référence avant le Deep Learning, atteindre recall needs repair $\geq$ 0,65.
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Paramètre & Valeur \\
\midrule
Données & data/cleaned/train\_clean.csv \\
Features & 26 (PumpPreprocessor.get\_feature\_columns()) \\
Split & 80/20 stratifié, random\_state=42 $\rightarrow$ 47 520 train / 11 880 test \\
...
\subsection{Modèles et gestion du déséquilibre}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Modèle & Stratégie déséquilibre \\
\midrule
...
\subsection{Résultats : baselines + GridSearch}

{\small
\begin{longtable}{@{}llllll@{}}
\toprule
\rowcolor{EHTPTableHead}Modèle & F1-Macro & F1 needs repair & Recall needs repair & Accuracy & ROC-AUC \\
\midrule
...
\subsection{Analyse par classe : RF champion F1}

{\small
\begin{longtable}{@{}lllll@{}}
\toprule
\rowcolor{EHTPTableHead}Classe & Precision & Recall & F1 & Support test \\
\midrule
...
\includegraphics[width=0.92\linewidth]{sprint_03_confusion_matrices.png}
...
\includegraphics[width=0.92\linewidth]{sprint_03_feature_importance.png}
...
\subsection{Recall boost : critère métier atteint}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Variante & F1-Macro & Recall needs repair & Seuil \\
\midrule
...
\chapter[Sprint 4]{Sprint 4 : Deep Learning et comparaison ML/DL}

\label{ch:sprint4}

\subsection{Objectif}

Tester si le Deep Learning dépasse le plafond \textasciitilde{}0,67 F1-Macro du ML et atteint F1 $\geq$ 0,72, tout en gardant recall $\geq$ 0,65.
...
\subsection{Résultats : toutes architectures DL}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Architecture & F1-Macro & Recall needs repair & Accuracy \\
\midrule
...
\includegraphics[width=0.92\linewidth]{sprint_04_training_history.png}
...
\includegraphics[width=0.92\linewidth]{sprint_04_confusion_matrix.png}

\caption{Matrice confusion MLP baseline}

\end{figure}

\subsection{Comparaison ML (S3) vs DL (S4)}

{\small
\begin{longtable}{@{}lllll@{}}
\toprule
\rowcolor{EHTPTableHead}Modèle & Type & F1-Macro & Recall needs repair & Accuracy \\
\midrule
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Écart & Delta \\
\midrule
...
\subsection{Pourquoi le DL n'a pas battu le ML ?}
...
\chapter[Sprint 5]{Sprint 5 : Arbitrage et modèles champions}
...
\subsection{Soft Voting RF + XGB (nouveauté S5)}
...
\begin{longtable}{@{}lllll@{}}
\toprule
\rowcolor{EHTPTableHead}Modèle & F1-Macro & Recall needs repair & Accuracy & ROC-AUC \\
\midrule
...
\subsection{Tableau comparatif global (extrait)}

{\small
\begin{longtable}{@{}lllll@{}}
\toprule
\rowcolor{EHTPTableHead}Modèle & Type & F1-Macro & Recall NR & Accuracy \\
\midrule
...
\subsection{Décision de production}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Priorité & Modèle & Fichier & Métrique clé \\
\midrule
...
\subsection{Model Card : synthèse}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Métrique & Valeur & Cible \\
\midrule
Recall needs repair & \textbf{0,6848} & $\geq$ 0,65 Oui \\
F1-Macro & 0,6295 & $\geq$ 0,72 Non \\
...
\rowcolor{EHTPTableHead}Métrique & Valeur & Cible \\
\midrule
F1-Macro & \textbf{0,6787} & $\geq$ 0,72 Non \\
Recall needs repair & 0,4855 & $\geq$ 0,65 Non \\
Accuracy & 0,770 & / \\
...
\chapter[Sprint 6]{Sprint 6 : Simulation IoT MQTT}
...
\subsection{Topics et payloads}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Direction & Topic & Format \\
\midrule
Simulateur $\rightarrow$ Broker & aquasense/\{pump\_id\}/telemetry & JSON \\
Consumer $\rightarrow$ Broker & aquasense/\{pump\_id\}/prediction & JSON \\
\bottomrule
\end{longtable}
}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Fichier & Rôle \\
\midrule
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Scénario & \textasciitilde{}Répartition & Comportement \\
\midrule
...
\textbf{Saison sèche Maroc :} mois 6–9 $\rightarrow$ débit réduit de 15 \% (DRY\_FLOW\_FACTOR = 0.85).

\textbf{Accélération :} SIM\_SECONDS\_PER\_DAY=60 $\rightarrow$ 1 min réelle = 1 jour simulé.

\subsection{Stratégie hybride profil ML + télémétrie}
...
\subsection{Résultats test 50 pompes}

\begin{figure}[htbp]

\centering

\includegraphics[width=0.92\linewidth]{image/fig_mqtt_consumer.png}
...
\includegraphics[width=0.92\linewidth]{image/fig_mqtt_simulator.png}
...
\chapter[Sprint 7]{Sprint 7 : Dashboard de supervision}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Composant & Fichier & Rôle \\
\midrule
...
\subsection{Captures probantes (session \textasciitilde{}107 578 messages MQTT)}

\begin{figure}[htbp]

\centering

\includegraphics[width=0.92\linewidth]{image/fig_dashboard_overview.png}
...
\includegraphics[width=0.92\linewidth]{image/fig_dashboard_alerts.png}
...
\includegraphics[width=0.92\linewidth]{image/fig_dashboard_pump.png}
...
\includegraphics[width=0.92\linewidth]{image/fig_dashboard_models1.png}
...
\includegraphics[width=0.92\linewidth]{image/fig_dashboard_models2.png}
...
\includegraphics[width=0.92\linewidth]{image/fig_dashboard_about.png}

\caption{À propos pipeline}

\end{figure}

\subsection{Synthèse métriques dashboard}
...
\subsection{Carte Maroc : clarification}
...
\chapter[Sprint 8]{Sprint 8 : Tests et validation}
...
\includegraphics[width=0.92\linewidth]{image/fig_pytest_all.png}
...
\includegraphics[width=0.92\linewidth]{image/fig_pytest_mqtt.png}

\caption{pytest intégration MQTT}

\end{figure}

\subsection{Couverture par fichier}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Fichier & Tests & Valide \\
\midrule
...
\subsection{Scénarios S8-01 à S8-06}

{\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}ID & Scénario & Résultat \\
\midrule
...
S8-05 & Recall maintenance & Oui $\geq$ 3/5 alertes détectées \\
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
\rowcolor{EHTPTableHead}Résultat & Valeur & Signification \\
\midrule
...
Latence MQTT & \textbf{22-70 ms} & Temps réel validé \\
...
\subsection{Reproduction complète}
...
\subsection{Fichiers modèles finaux}
...
\textit{Rapport rédigé par TRAORE Fanogo Mohamed \textbackslash\{\}\& NADAHE Mohamed ,  Projet ML EHTP ,  Juin 2026}
\end{document}
